<template>
  <div>
    <div class="insight" id="promo-banner">
      <strong><span v-html="BP_ICONS.pin"></span> 推广总览：</strong><span class="mono">花费 {{ bpMoney(banner.spend) }}</span> · 成交 <span class="mono">{{ bpMoney(banner.pay) }}</span> · ROI <span class="mono">{{ banner.spend ? (banner.pay / banner.spend).toFixed(1) : '--' }}</span><span class="ghost" style="margin-left:8px;">（单日快照 {{ bp.lastDay.value }} · 含零成交计划 · 明细仅列有花费计划 TOP{{ bp.promoTop.value.length }}）</span>
    </div>
    <div class="promo-list" id="promo-list">
      <div v-for="x in bp.promoTop.value" :key="x.pid" class="promo-item2" :class="{ open: openPromo.has(x.pid) }" :data-pid="x.pid">
        <button class="promo-head2" @click="toggle(x.pid)">
          ...
          <span class="promo-stats2 mono">花费 {{ bpMoney(x.spend) }} | 成交 {{ bpMoney(x.pay) }} | ROI {{ x.roi != null ? x.roi.toFixed(1) : '--' }}</span>
        ...
        <div class="promo-body2">
          <div class="promo-body2-inner">
            ...
            <table class="promo-detail2" style="margin-top:12px;">
              <thead><tr><th>日期</th><th>花费</th><th>点击</th><th>展现</th><th>点击率</th><th>成交金额</th><th>成交笔数</th><th>ROI</th><th>转化</th></tr></thead>
              <tbody>
                <tr>
                  ...
                  <td class="num">{{ x.cr != null ? bpPct(x.cr * 100) : '--' }}</td>
                  ...
                  <td class="num">{{ x.conv != null ? bpPct(x.conv * 100) : '--' }}</td>
                </tr>
              </tbody>
            </table>
            <div class="pd-note" style="margin-top:8px;">推广明细数据源为 {{ bp.lastDay.value }} 单日（商品报表），模板未导出多日推广明细；店铺级分日推广花费见 02/03 模块聚合。</div>
          </div>
        </div>
      </div>
    </div>
